...
{
#include "ft_app_ic_oufei_boe.txt"
};
...
static int fts_ctpm_fw_upgrade_with_i_file(struct ft5x06_ts_data *data)
{
	...
	else if (TP_Maker== TP_OUFEI&& LCD_Maker == 0x35 && Project_Id == PROJECT_C3B){
		pbt_buf = CTPM_FW8;
		fw_len = sizeof(CTPM_FW8);
		printk("ttt TP_OUFEI+ BOE");
	}else{
...
static ssize_t ctp_lockdown_proc_read(struct file *file, char __user *buf,size_t count, loff_t *ppos)
{
	...
	*ppos += count;
	...
	if (copy_to_user(buf, ptr, ret+1)) {
		CTP_INFO("%s: fail to copy default config\n", __func__);
		ret = -EFAULT;
	    }
	kfree(ptr);
	return  ret;
...
static ssize_t ctp_open_proc_read(struct file *file, char __user *buf,size_t count, loff_t *ppos)
{
	...
	*ppos += count;
	...
	if (TP_Maker== TP_OUFEI&& LCD_Maker == TP_EBBG&&Project_Id == PROJECT_C3B) {
		...
		sprintf(cfgname, "%s", "ft5346_oufei_boe.ini");
		printk("ttt cfgname is: %s\n", cfgname);
	} else {
	...
	nret=sprintf(ptr, "result=%d\n",result);

	if (copy_to_user(buf, ptr, nret+1)) {
		CTP_INFO("%s: fail to copy default config\n", __func__);
		nret = -EFAULT;
...
static ssize_t proc_gesture_data_read(struct file *file, char __user *buffer, size_t count, loff_t *ppos)
{
	...
	if (copy_to_user(buffer, cmd, ret+1)) {

		CTP_ERROR("%s: fail to copy default config\n", __func__);
		ret = -EFAULT;
...
static ssize_t proc_gesture_data_write(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
	...
	if (buffer != NULL) {
	    res = copy_from_user(cmd, buffer, count - 1);
		if (res < 0) {
			CTP_ERROR("copy data from user space, failed\n");
			return res;
		}
	}
	sscanf(cmd,"%c",&gtp_gesture_value);
	return count;
}
...
static ssize_t proc_gesture_onoff_read(struct file *file, char __user *page, size_t count, loff_t *ppos)
{
	...
	if (copy_to_user(page, cmd, ret+1)) {
...
static ssize_t proc_gesture_onoff_write(struct file *file,  const char __user *buffer, size_t count, loff_t *ppos)
{
	...
	sscanf(cmd,"%c",&gtp_gesture_onoff);
	return count;
}
...
/*Gesture,@hubing1.wt,Begin*/
	input_dev->event = fts_input_event;
/*Gesture,@hubing1.wt,End*/
